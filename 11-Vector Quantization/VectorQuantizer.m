...
%%    
%         AVG = sum / num ;
%         z = randfixedsum(m*n,1,AVG*m*n,0,255); 
%         CB(:,:,i)=floor(reshape(z,m,n));


daten=zeros(size(data,1),size(data,2));
for i=1:k
    for r=1:num_rows
        for c=1:num_cols
            meee = mean2(data((r-1)*block_size+1:r*block_size, (c-1)*block_size+1:c*block_size));
            if(meee>=vor(i)&&meee<vor(i+1))
